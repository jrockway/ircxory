...
$schema->deploy;
bless $schema => 'App::Ircxory::Robot::DBLogger'; # ick.

isa_ok($schema, 'App::Ircxory::Robot::DBLogger');

my $person;
{
    my $nick = $schema->get_nickname('jrockway', 'jon', 'jrock.us');
    ...
    $nick = $schema->get_nickname('jrockway', 'jon', 'foo.jrock.us');
    is($nick->nick, 'jrockway');
    ok($nick->person, 'have a person');
    is($nick->person->name, 'jrockway', 'created a person with nick as name');
