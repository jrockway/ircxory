...
sub highest {
    map { [$_->thing, $_->total_points] }
      $schema->resultset('Things')->highest_rated(@_);
}
...
my @lowfour = map {[ $_->thing, $_->total_points ]} 
  $schema->resultset('Things')->lowest_rated(4);

is_deeply(\@botfour, [@BOTTOM_TEN[0..3]], 'got lowest four');

my @lowten = map {[ $_->thing, $_->total_points ]} 
  $schema->resultset('Things')->lowest_rated();
